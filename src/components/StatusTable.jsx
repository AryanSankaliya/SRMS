function StatusTable({ data = [], role }) {
    ...
    return (
        <div className='bg-white rounded-3xl shadow-md p-6 w-full mt-5'>
            {/* Header */}
            <div className={"grid  grid-cols-5 text-sm text-teal-600 font-semibold pb-4 border-b"}>
                <div>REQUEST NO</div>
                <div>TITLE</div>
                <div>TYPE</div>
                <div>STATUS</div>
            ...
            {/* Rows */}
            {data.map((item, index) => (
                <div key={index} className="grid grid-cols-5 items-center py-4 border-b last:border-none hover:bg-gray-50 transition">
                    ...
                    {/* {showFinalStatus && (
                        <div className={`px-3 py-1 w-16 text-sm rounded-full ${item.final_status === "Open" ? "bg-green-100 text-green-600" : "bg-gray-200 text-gray-700"}`}>
                            {item.final_status}
                        </div>
                    )} */}
                </div>
